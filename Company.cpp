...
void Company::Load_Info(const string File_name)
{
	...
	Special_Truck st;
	VIP_Truck vt;
	nt.set_J(Journey_Limit_CheckUp);
	st.set_J(Journey_Limit_CheckUp);
	vt.set_J(Journey_Limit_CheckUp);
	nt.SetCheckupTime(Normal_CheckUP_Time);
	st.SetCheckupTime(Normal_CheckUP_Time);
	vt.SetCheckupTime(Normal_CheckUP_Time);
	// 4 line
...
void Company::LoadCargo()
{
	...
	if (Waiting_Normal_Cargos.getcount() >= Normal_Capacity )
	{

		Load_Normal_Cargos();
	}
	if (Waiting_Special_Cargos.getcount() >= Special_Capacity )
	{
		Load_Special_Cargos();
	}
}
...
{
	//if (offHours(Company_Time.transfHours()))
	//{
		LoadCargo();
		Loading_To_Moving();
	//}
...
{   //////// you may need to change all ''moving'' to "loading " only 
	...
	{
		Moving_Trucks.peek(temp); // we look at the peek 
		if (temp->GetDeliveryInterval()+temp->GetMoveTime().transfHours()<= Company_Time.transfHours()) { // if its the time to be back we will choose whether
			...
			else if (temp->Need_to_checkup()) { // truck to checkup queue
				temp->set_start_check(Company_Time);
				if (dynamic_cast<Normal_Truck*>(temp)) {
					temp->set_end_check(Company_Time + Normal_Truck::get_checkUpTime());
					InCheckup_Normal_Trucks.enqueue(temp);
...
void Company::In_NumNormCargos()
{
	NumNormCargos++;
}

void Company::In_NumSpecCargos()
{
	NumSpecCargos++;
}

void Company::In_NumVIPCargos()
{
	NumVIPCargos++;
}

void Company::In_Num_AutoPromote()
{
	Num_AutoPromote++;
}