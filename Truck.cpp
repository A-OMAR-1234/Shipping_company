...
int Truck::GetUtilization(Time Simulation_Time)
{
	...
	if (Total_Delivery_journeys == 0)
		...
		return Total_Delivered_Cargos / ((Capacity * Total_Delivery_journeys) * (Active_Time.transfHours() / Simulation_Time.transfHours()));
}
...
bool Truck::Need_to_checkup()
{
	if (Total_Delivery_journeys % num_J_to_check == 0) 
	{
		return true;
	}
	return false;
